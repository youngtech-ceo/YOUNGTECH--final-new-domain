...
// Wrapped in an IIFE to prevent global namespace collisions with script.js (e.g. firebaseConfig, database)
...
(function() {
    ...
    // Reuse existing database reference if initialized globally by script.js
    ...
    function handleInitialRouting() {
        ...
        if (postId && blogsCache[postId]) {
            showPostReader(postId, false);
        } else {
            closePostReader(false);
        }
    }
    ...
    // Attach local functions to global window object so HTML buttons (onclick) can access them
    window.showPostReader = showPostReader;
    window.closePostReader = closePostReader;
